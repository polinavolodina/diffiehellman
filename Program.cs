...
namespace methods2
{
    class Program
    {
        ...
        public static BigInteger p, B, NR, r;
        ...
        private static void Start()
        {
            ...
            Console.WriteLine("\t2 - Если хотите установить сеансовый ключ для абонентов А и В");
        ...
        public static void Protocol()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    Console.WriteLine("\t1 - Абонент А генерирует случайное число a и вычисляет точку aQ");
                    Console.WriteLine("\t2 - Абонент B генерирует случайное число b и вычисляет точку bQ");
                    Console.WriteLine("\t3 - Вычисление абонентом А итогового сеансового ключа");
                    Console.WriteLine("\t4 - Вычисление абонентом В итогового сеансового ключа");
                    Console.WriteLine("\t5 - Проверка итогового сеансового ключа");
                    Console.WriteLine("\t0 - Выход");
                    int mode;
                    flag = true;
                    try
                        ...
                        switch (mode)
                        {
                            ...
                            case 3:
                                ...
                                case 5:
                                {
                                    ...
                                    if (!Step5()) 
                                    {
                                        Console.WriteLine("На каком-то шаге произошла ошибка, общий сеансовый ключ не установлен");
                                        flag2 = false;
                                        return;
                                    }
                                    else
                                    {
                                        Console.WriteLine("Cеансовый ключ установлен");
                                    }
                                    step = 5;
                                    using (var write = new StreamWriter(File.Open("Step.txt", FileMode.OpenOrCreate)))
                                    {
        ...
        public static void Step1()
        {
            Q.Clear();
            ReadOptions();
            BigInteger a = rand.Next(2, (int)r);
            
            var aQ = QuickSumPoint(Q, a, B, p);

            try
            {
                using (var write = new StreamWriter(File.Open("a.txt", FileMode.OpenOrCreate)))
                {
                    write.WriteLine(a);
                }

                using (var write = new StreamWriter(File.Open("aQ.txt", FileMode.Create)))
                {
                    write.WriteLine(aQ[0]);
                    write.WriteLine(aQ[1]);
                }
            }
            catch (FileNotFoundException)
            {
        ...
        public static void Step2()
        {
            Q.Clear();
            ReadOptions();
            BigInteger b = rand.Next(2, (int)r);
            
            var bQ = QuickSumPoint(Q, b, B, p);
            ...
            try
            {
                ...
                using (var write = new StreamWriter(File.Open("bQ.txt", FileMode.Create)))
                {
                    write.WriteLine(bQ[0]);
                    write.WriteLine(bQ[1]);
                }
            }
            catch (FileNotFoundException)
            {
        ...
        public static void Step3()
        {
            try
            {
                var bQ_a = new List<BigInteger>();
                using (var sr = new StreamReader(File.Open("bQ.txt", FileMode.Open), Encoding.Default))
                {
                    bQ_a.Add(BigInteger.Parse(sr.ReadLine()));
                    bQ_a.Add(BigInteger.Parse(sr.ReadLine()));
                }
                var a = BigInteger.Parse(File.ReadAllText("a.txt"));
                var bQa = QuickSumPoint(bQ_a, a, B, p);
                using (var write = new StreamWriter(File.Open("bQa.txt", FileMode.Create)))
                {
                    write.WriteLine(bQa[0]);
                    write.WriteLine(bQa[1]);
                }
            }
            catch (FileNotFoundException)
            {
        ...
        public static void Step4()
        {
            try
            {
                var aQ_b = new List<BigInteger>();
                using (var sr = new StreamReader(File.Open("aQ.txt", FileMode.Open), Encoding.Default))
                {
                    aQ_b.Add(BigInteger.Parse(sr.ReadLine()));
                    aQ_b.Add(BigInteger.Parse(sr.ReadLine()));
                }
                var b = BigInteger.Parse(File.ReadAllText("b.txt"));
                var aQb = QuickSumPoint(aQ_b, b, B, p);
                using (var write = new StreamWriter(File.Open("aQb.txt", FileMode.Create)))
                {
                    write.WriteLine(aQb[0]);
                    write.WriteLine(aQb[1]);
                }
            }
            catch (FileNotFoundException)
            {
        ...
        public static bool Step5()
        {
            try
            {
                var bQa = new List <BigInteger>();
                using (var sr = new StreamReader(File.Open("bQa.txt", FileMode.Open), Encoding.Default))
                {
                    bQa.Add(BigInteger.Parse(sr.ReadLine()));
                    bQa.Add(BigInteger.Parse(sr.ReadLine()));
                }

                var aQb = new List <BigInteger>();
                using (var sr = new StreamReader(File.Open("aQb.txt", FileMode.Open), Encoding.Default))
                {
                    aQb.Add(BigInteger.Parse(sr.ReadLine()));
                    aQb.Add(BigInteger.Parse(sr.ReadLine()));
                }
                if (bQa[0] != aQb[0] || bQa[1] != aQb[1]) 
                    return false;
                return true;
            }
            catch (FileNotFoundException)
                        ...
                        case 1:
                            {
                                foreach (var item in Directory.GetFiles(Environment.CurrentDirectory))
                                {
                                    if (Path.GetFileName(item) == "ElipticCurve.txt") 
                                        continue;
                                    if (Path.GetExtension(item) == ".txt")
                                        File.Delete(item);
                                }
